...
*/
void iterate(int maxc,int minc, int X_RESN, int Y_RESN, int* TheK, int max_iteration)
{
  int i,j;
...
if (argv[1]!=NULL){
  ...
  */
  ...
  clock_t t1, t2;
        t1=clock();
           compute(X_RESN/p,Y_RESN/p, max_iteration);
  t2=clock();   
...
void compute(int X_RESN, int Y_RESN, int max_iteration)
{
  ...
  iterate(maxc, minc, X_RESN, Y_RESN,TheK, max_iteration);
