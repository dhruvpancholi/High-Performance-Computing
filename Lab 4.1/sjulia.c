...
void compute(int X_RESN, int Y_RESN, char const *argv[]);
void iterate(int minc, int maxc, int X_RESN, int Y_RESN, int *kk, char const *argv[]);
...
void compute(int X_RESN, int Y_RESN, char const *argv[])
{
	...
	iterate(maxc, minc, X_RESN, Y_RESN,TheK, argv);
...
*/
void iterate(int maxc,int minc, int X_RESN, int Y_RESN, int* TheK, char const *argv[])
{
  int i,j;
  ...
  for( i = 0; i <  Y_RESN ; i++)
        ...
        for(j=0; j < X_RESN; j++)
        {
            ...
            z.real   = -2 + (4.0/ (X_RESN-1) )*(CENTER_X+0.5*((double) j)) ;
            z.imag   = -2 - (4.0/ (Y_RESN-1) )*( 0.5*((double) i) - (Y_RESN - 1) ); 
